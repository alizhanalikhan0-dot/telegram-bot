...
TOKEN = "ВАШ_ТОКЕН_ЗДЕСЬ"      # сюда вставь свой токен
...
async def user_msg(message: types.Message):
    ...
    if message.photo:
        ...
        await bot.send_voice(ADMIN_ID, message.voice.file_id,
                             caption=f"Голосовое от {user_id}", reply_markup=kb.as_markup())
    elif message.video:
        await bot.send_video(ADMIN_ID, message.video.file_id,
                             caption=f"Видео от {user_id}", reply_markup=kb.as_markup())
    elif message.document:
        await bot.send_document(ADMIN_ID, message.document.file_id,
                                caption=f"Документ от {user_id}", reply_markup=kb.as_markup())
    else:
...
async def admin_reply(message: types.Message):
    ...
    if message.text:
        ...
        await message.answer("Отвечать можно только текстом.")
